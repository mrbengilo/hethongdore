...
test("migration chain resets all operational data while preserving manager credentials and session", async () => {
  ...
  db.exec(`
    INSERT INTO stores (id, name, address, revenue, expense, status, created_at)
      VALUES ('store-1', 'Store 1', 'Address', 5000000, 2000000, 'ACTIVE', '2026-08-01T00:00:00Z');
  ...
  const resetSql = await sql("0007_reset_operational_data.sql");
  db.exec(resetSql);
  db.prepare("INSERT INTO system_state (key, value, updated_at) VALUES (?, ?, ?)")
    .run("future_state", "KEEP", "2026-08-08T00:00:00Z");
  db.exec(resetSql);
  ...
  assert.deepEqual(db.prepare("SELECT id, role, password_hash AS passwordHash, employee_id AS employeeId, store_id AS storeId, failed_attempts AS failedAttempts, locked_until AS lockedUntil, shift_active AS shiftActive, current_shift AS currentShift, shift_started_at AS shiftStartedAt FROM users ORDER BY id").all().map((row) => ({ ...row })), [
    {
      id: "manager-1",
      role: "MANAGER",
  ...
  assert.deepEqual(db.prepare("SELECT id, user_id AS userId FROM sessions ORDER BY id").all().map((row) => ({ ...row })), [
    { id: "manager-session", userId: "manager-1" },
    { id: "manager-session-2", userId: "manager-2" },
  ]);
...
test("runtime keeps clean bootstrap, one-time reset fallback and scoped CCCD R2 cleanup", async () => {
  const [runtime, schema, journal] = await Promise.all([
  ...
  assert.match(runtime, /DELETE FROM system_state WHERE key = \?/u);
  ...
  assert.match(journal, /0007_reset_operational_data/u);
});
