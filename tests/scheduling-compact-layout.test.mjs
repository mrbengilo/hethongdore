...
test("mobile scheduling surfaces remain scrollable, named and viewport-safe", async () => {
  const [globalStyles, styles, source] = await Promise.all([
    readFile(globalStylesUrl, "utf8"),
    readFile(stylesUrl, "utf8"),
    readFile(sourceUrl, "utf8"),
  ]);

  assert.match(globalStyles, /@media\(max-width:720px\)\{[\s\S]*?\.sidebar\{[^}]*overflow-y:auto;[^}]*overscroll-behavior:contain;[^}]*max-height:100vh;[^}]*max-height:100dvh;[^}]*padding-bottom:max\(16px,env\(safe-area-inset-bottom\)\)/u);
  assert.match(styles, /\.modal\s*\{[^}]*max-height:\s*calc\(100vh - 36px\);[^}]*max-height:\s*calc\(100dvh - 36px\);[^}]*overflow-y:\s*auto;/su);
  assert.match(source, /className=\{styles\.tableWrap\}\s+role="region"\s+tabIndex=\{0\}\s+aria-label="Lịch phân ca theo ngày, cuộn ngang để xem đầy đủ"/u);
  assert.match(source, /className=\{styles\.tableWrap\}\s+role="region"\s+tabIndex=\{0\}\s+aria-label="Lịch phân ca theo tuần, cuộn ngang để xem đầy đủ"/u);
  assert.match(styles, /\.employeeName b\s*\{[^}]*overflow:\s*hidden;[^}]*text-overflow:\s*ellipsis;[^}]*white-space:\s*nowrap;/su);
  assert.match(source, /<b title=\{employee\.name\}>\{employee\.name\}<\/b><small title=\{`\$\{employee\.code\} · \$\{employee\.position\}`\}>/u);
});
...
test("schedule grids keep the employee column compact and scroll inside their panel", async () => {
  ...
  assert.doesNotMatch(styles, /\.scheduleTable:not\(\.weekTable\) \.employeeName b,[\s\S]*?\.scheduleTable:not\(\.weekTable\) \.employeeName small\s*\{[^}]*overflow:\s*visible;[^}]*text-overflow:\s*clip;/u);
