...
export function PayrollPolicySettings() {
  ...
  return <section className={styles.card} aria-labelledby="payroll-policy-title">
    ...
    <ActionForm onSubmit={submit}>
      ...
      <fieldset className={styles.wide}><legend><Percent size={19}/> Tỷ lệ chia lợi nhuận cuối cùng</legend><div className={memberStyles.memberHeader}><small>Chỉ lợi nhuận sau cùng dương của từng cửa hàng đã khóa mới được phân chia. Tổng tỷ lệ phải đúng 100%.</small><ActionButton type="button" onClick={() => setProfitSharingMembers((current) => [...current, { memberId: crypto.randomUUID(), name: "", ratePercent: 0 }])}><Plus size={16}/> Thêm người nhận</ActionButton></div><div className={memberStyles.members}>{profitSharingMembers.length === 0 ? <p className={memberStyles.memberEmpty}>Chưa cấu hình người nhận. Hệ thống sẽ không cho chốt chia lợi nhuận cho đến khi danh sách hợp lệ được lưu.</p> : profitSharingMembers.map((member, index) => <div className={memberStyles.memberRow} key={member.memberId}><label>Họ tên<input required value={member.name} maxLength={160} onChange={(event) => setProfitSharingMembers((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, name: event.target.value } : item))}/></label><label>Tỷ lệ<div className={styles.percentInput}><input type="number" inputMode="decimal" min="0" max="100" step="0.01" required value={member.ratePercent} onChange={(event) => setProfitSharingMembers((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, ratePercent: Number(event.target.value) } : item))}/><span>%</span></div></label><ActionButton className={memberStyles.removeMember} type="button" aria-label={`Xóa người nhận ${member.name || index + 1}`} onClick={() => setProfitSharingMembers((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={17}/></ActionButton></div>)}</div><small>Tổng đang nhập: {profitSharingMembers.reduce((total, member) => total + Number(member.ratePercent || 0), 0).toLocaleString("vi-VN", { maximumFractionDigits: 2 })}%.</small></fieldset>
      ...
      <div className={styles.actions}><p>Phiên bản {data.policy.version} · cập nhật {dateTime(data.policy.updatedAt)}{data.policy.updatedByName ? ` bởi ${data.policy.updatedByName}` : ""}</p><ActionButton type="submit" disabled={saving}><Save size={17}/> {saving ? "Đang lưu..." : "LƯU CHÍNH SÁCH"}</ActionButton></div>
