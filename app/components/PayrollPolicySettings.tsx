...
type PolicyResponse = {
  policy?: {
    ...
    employeeTikTokAllowanceVnd: number | null;
...
export function PayrollPolicySettings() {
  ...
  const [tikTokAllowance, setTikTokAllowance] = useState("");
  ...
  const hydrate = useCallback((body: PolicyResponse) => {
    ...
    setTikTokAllowance(body.policy.employeeTikTokAllowanceVnd === null
      ? ""
      : formatVndInput(body.policy.employeeTikTokAllowanceVnd));
  ...
  async function submit(event: FormEvent) {
    ...
    const employeeTikTokAllowanceVnd = parseVndInput(tikTokAllowance);
    ...
    if (!Number.isSafeInteger(employeeTikTokAllowanceVnd) || employeeTikTokAllowanceVnd < 0) {
      setError("Phụ cấp TikTok mặc định phải là số nguyên từ 0 đồng trở lên."); return;
    }
  ...
  return <section className={styles.card} aria-labelledby="payroll-policy-title">
    ...
    <form onSubmit={submit}>
      ...
      <fieldset><legend><BadgeDollarSign size={19}/> Phụ cấp TikTok mặc định</legend><label htmlFor="employee-policy-tiktok">Mức phụ cấp cho mỗi ca có TikTok</label><div className={styles.moneyInput}><input id="employee-policy-tiktok" type="text" inputMode="numeric" pattern="[0-9,]*" required value={tikTokAllowance} onChange={(event) => setTikTokAllowance(formatVndInput(event.target.value))}/><span>đồng</span></div><small>Ca làm mới sẽ chụp lại đúng mức đang áp dụng; lịch sử không đổi khi cập nhật chính sách.</small></fieldset>
