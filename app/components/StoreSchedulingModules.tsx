...
const defaultShifts: ShiftDefinition[] = [
  { id: "default-1", name: "Ca 1", start: "07:00", end: "15:00", duration: 480, overnight: false, persisted: false },
  { id: "default-2", name: "Ca 2", start: "15:00", end: "22:00", duration: 420, overnight: false, persisted: false },
  { id: "default-3", name: "Ca 3", start: "22:00", end: "07:00", duration: 540, overnight: true, persisted: false },
];
...
function useEmployees(storeId: string) {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/employees?storeId=${encodeURIComponent(storeId)}`);
      const result = await response.json();
...
export function StoreShiftManagement({ store }: { store: SchedulingStore }) {
  ...
  const shifts = useMemo(() => {
    ...
    return persisted.length ? persisted : defaultShifts;
...
export function StoreScheduleManagement({ store }: { store: SchedulingStore }) {
  ...
  return <section className={styles.shell} aria-label="Quản lý lịch phân ca">
    ...
    <section className={styles.panel}>
      <div className={styles.panelHeader}><div className={styles.tabs}><button type="button" className={view === "day" ? styles.activeTab : ""} onClick={() => setView("day")}>Theo ca</button><button type="button" className={view === "week" ? styles.activeTab : ""} onClick={() => setView("week")}>Theo tuần</button><button type="button" className={view === "employee" ? styles.activeTab : ""} onClick={() => setView("employee")}>Theo nhân viên</button></div><div className={styles.dateNav}><button type="button" onClick={() => setDate(addDays(date, view === "day" ? -1 : -7))}><ChevronLeft size={18}/></button><b>{view === "day" ? dateLabel(date) : `${shortDate(week[0])} - ${shortDate(week[6])}`}</b><button type="button" onClick={() => setDate(addDays(date, view === "day" ? 1 : 7))}><ChevronRight size={18}/></button></div></div>
    ...
    {open && <div className={styles.backdrop}><form className={`${styles.modal} ${styles.scheduleModal}`} onSubmit={save}>
      ...
      {step === 1 ? <div className={styles.stepBody}>
        ...
        {message && <p className={styles.error}>{message}</p>}
        <div className={styles.modalActions}><button type="button" className={styles.secondaryButton} onClick={() => setStep(1)}><ChevronLeft size={17}/> Quay lại</button><button className={styles.primaryButton} disabled={saving || inactive || !selectedEmployees.length}>{saving ? "Đang lưu..." : editing ? "Cập nhật lịch" : "Lưu lịch phân ca"}</button></div>
      </div>}
    </form></div>}
  </section>;
}
