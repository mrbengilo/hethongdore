...
type PayrollAction = "FINALIZE_EMPLOYEE" | "FINALIZE_MANAGER" | "CONFIRM_PAYMENT" | "CLOSE_PERIOD";
...
export default function StorePayrollClosing({ store, initialPeriod }: { store: Store; initialPeriod?: string }) {
  ...
  const nextAction = useMemo<{ action: PayrollAction; label: string } | null>(() => {
    ...
    if (closing.status === "MANAGER_FINALIZED") return { action: "CONFIRM_PAYMENT", label: "Xác nhận đã chi" };
  ...
  return <div className="reference-module payroll-page">
    ...
    {summary && <>
      ...
      <section className="manager-panel">
        ...
        <div className="comparison-grid">
          ...
          <p><span>3. Xác nhận chi trả</span><b>{closing?.status === "PAYMENT_CONFIRMED" || closing?.status === "LOCKED" ? "Đã xác nhận" : "Chờ xác nhận"}</b><em>{money(grandTotal)}</em></p>
          <p><span>4. Khóa kỳ</span><b>{closing?.status === "LOCKED" ? "Đã khóa" : "Chưa khóa"}</b><em>{period}</em></p>
        </div>
      ...
      <div className="comparison-grid">
        ...
        <section className="manager-panel"><h2>ĐỐI SOÁT KỲ HIỆN TẠI</h2>
          ...
          <p><span>Lợi nhuận</span><b>{money(summary.profit)}</b><em>{money(summary.profitPerHour)}/giờ</em></p>
    ...
    <section className="manager-panel table-panel">
      ...
      <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Kỳ</th><th>Cửa hàng</th><th>Nhân viên</th><th>Lương quản lý</th><th>Thưởng quản lý</th><th>Tổng chi</th><th>Trạng thái</th><th>Thời gian khóa</th></tr></thead><tbody>
        {!data.history?.length ? <tr><td colSpan={8} className="empty-cell">Chưa có lịch sử kết sổ.</td></tr> : data.history.map((item) => <tr key={`${item.storeId}-${item.period}`}><td><b>{item.period}</b></td><td>{item.storeName}</td><td>{money(item.employeeTotal)}</td><td>{money(item.managerSalary)}</td><td>{money(item.managerBonus)}</td><td className="money-green"><b>{money(item.grandTotal)}</b></td><td><span className="status-pill">{statusLabel(item.status)}</span></td><td>{item.closedAt ? new Date(item.closedAt).toLocaleString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" }) : "—"}</td></tr>)}
      </tbody></table></div>
    </section>
  </div>;
}
