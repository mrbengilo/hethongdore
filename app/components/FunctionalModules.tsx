...
export function FunctionalManagerPayroll({ stores }: { stores: FunctionalStore[] }) {
  return <div className="page-content">
    <section className="form-card">
      <h2>Lương thưởng quản lý được lấy từ kỳ đã khóa</h2>
      <div className="notice-banner" role="note">Màn hình cũ này chỉ cung cấp hướng dẫn và không được tạo bảng lương thủ công. Hãy dùng danh mục <b>Lương thưởng quản lý</b> để xem số liệu đã xác nhận chi và khóa sổ.</div>
      <div className="payroll-guide">
        <p>Lương quản lý được lấy từ <b>chính sách có phiên bản áp dụng cho kỳ</b>.</p>
        <p>KPI được tính bởi <b>Finance Engine dùng chung</b>; giao diện không tự đặt số giờ hoặc tỷ lệ.</p>
        <p>Khi kỳ đã khóa, hệ thống chỉ đọc <b>snapshot bất biến</b> của kỳ đó.</p>
      </div>
    </section>
    <section className="table-card">
      <div className="table-head"><h2>Cửa hàng áp dụng chính sách</h2><span>{stores.length} cửa hàng</span></div>
      <table className="data-table">
        <thead><tr><th>Cửa hàng</th><th>Chính sách áp dụng</th><th>Nguồn số liệu</th></tr></thead>
        <tbody>{stores.length === 0
          ? <tr><td colSpan={3} className="empty-cell">Chưa có cửa hàng.</td></tr>
          : stores.map((store) => <tr key={store.id}><td><b>{store.name}</b></td><td>Theo phiên bản của kỳ</td><td>Finance Engine / snapshot kỳ đã khóa</td></tr>)}</tbody>
      </table>
    </section>
  </div>;
}
...
export function FunctionalEmployees({ store }: { store: FunctionalStore }) {
  const empty = { id: "", code: "", name: "", position: "Nhân viên bán hàng", phone: "", hourlyRate: formatVndInput(20000), username: "", password: "" };
  ...
  useEffect(() => { reload(); }, [reload]);
  function edit(item?: Employee) { setForm(item ? { id: item.id, code: item.code, name: item.name, position: item.position, phone: item.phone, hourlyRate: formatVndInput(item.hourly_rate), username: item.username ?? "", password: "" } : empty); setMessage(""); setOpen(true); }
  async function save(event: FormEvent) { event.preventDefault(); const response = await fetch("/api/employees", { method: form.id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, hourlyRate: parseVndInput(form.hourlyRate), storeId: store.id }) }); const result = await response.json(); if (!response.ok) return setMessage(result.message); setOpen(false); await reload(); }
  ...
  return <><div className="toolbar"><div className="stats-inline"><b>{employees.length}</b> nhân viên đang quản lý</div><ActionButton className="primary-button" onClick={() => edit()}><Plus size={17}/> Thêm nhân viên</ActionButton></div><div className="table-card"><div className="table-head"><h2>Danh sách nhân viên</h2></div><table className="data-table"><thead><tr><th>Mã NV</th><th>Họ tên</th><th>Chức vụ</th><th>SĐT</th><th>Lương/giờ</th><th>Tài khoản</th><th>Thao tác</th></tr></thead><tbody>{employees.map((employee) => <tr key={employee.id}><td><b>{employee.code}</b></td><td>{employee.name}</td><td>{employee.position}</td><td>{employee.phone}</td><td>{money(employee.hourly_rate)}</td><td>{employee.username ?? "—"}</td><td><div className="row-actions"><ActionButton onClick={() => edit(employee)}><Pencil size={15}/> Sửa</ActionButton><ActionButton className="danger" onClick={() => archive(employee.id)}><Trash2 size={15}/> Xóa</ActionButton></div></td></tr>)}</tbody></table></div>{open && <div className="modal-backdrop"><ActionForm className="modal" onSubmit={save}><div className="modal-title"><h2>{form.id ? "Cập nhật nhân viên" : "Thêm nhân viên"}</h2><ActionButton type="button" onClick={() => setOpen(false)}>×</ActionButton></div><div className="form-grid two"><label>Mã nhân viên<input required value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })}/></label><label>Họ tên<input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}/></label><label>Chức vụ<input required value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })}/></label><label>Số điện thoại<input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}/></label><label>Lương theo giờ<input type="text" inputMode="numeric" pattern="[0-9,]*" required value={form.hourlyRate} onChange={(e) => setForm({ ...form, hourlyRate: formatVndInput(e.target.value) })}/></label><label>Tên đăng nhập<input disabled={Boolean(form.id)} required={!form.id} value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })}/></label><label>Mật khẩu {form.id && "(để trống nếu giữ nguyên)"}<input type="password" required={!form.id} minLength={6} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}/></label></div>{message && <div className="form-message">{message}</div>}<div className="modal-actions"><ActionButton type="button" onClick={() => setOpen(false)}>Hủy</ActionButton><ActionButton className="primary-button">Lưu nhân viên</ActionButton></div></ActionForm></div>}</>;
}
...
export function FunctionalStoreRecords({ store, view }: { store: FunctionalStore; view: string }) {
  ...
  function begin(record?: BusinessRecord) { setEditing(record ?? null); setTitle(record?.title ?? ""); setAmount(formatVndInput(record?.data.amount as string | number | null | undefined)); setDate(String(record?.data.date ?? today())); setNote(String(record?.data.note ?? "")); setMessage(""); setOpen(true); }
  async function save(event: FormEvent) { event.preventDefault(); const payload = { id: editing?.id, category, storeId: store.id, title, data: { amount: parseVndInput(amount), date, note } }; const response = await fetch("/api/records", { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json(); if (!response.ok) return setMessage(result.message); setOpen(false); reload(); }
  const total = records.reduce((sum, record) => sum + Number(record.data.amount ?? 0), 0);
  return <><div className="stats-grid three"><div className="stat-card green"><div><span>TỔNG BẢN GHI</span><strong>{records.length}</strong><small>Dữ liệu đã lưu</small></div></div><div className="stat-card orange"><div><span>TỔNG GIÁ TRỊ</span><strong>{money(total)}</strong><small>Theo danh mục</small></div></div><div className="stat-card blue"><div><span>CẬP NHẬT</span><strong>{records[0] ? dateTime(records[0].updated_at) : "—"}</strong><small>Gần nhất</small></div></div></div><div className="table-card"><div className="table-head"><h2>Chi tiết {view.toLowerCase()}</h2><div><ActionButton onClick={() => downloadCsv(`${category.toLowerCase()}.csv`, [["Ngày", "Nội dung", "Số tiền", "Ghi chú"], ...records.map((r) => [String(r.data.date ?? ""), r.title, Number(r.data.amount ?? 0), String(r.data.note ?? "")])])}><Download size={16}/> Xuất Excel</ActionButton><ActionButton className="primary-button" onClick={() => begin()}><Plus size={16}/> Thêm mới</ActionButton></div></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Ngày</th><th>Nội dung</th><th>Số tiền/Giá trị</th><th>Ghi chú</th><th>Thao tác</th></tr></thead><tbody>{records.length === 0 ? <tr><td className="empty-cell" colSpan={5}>Chưa có dữ liệu. Bấm “Thêm mới” để bắt đầu.</td></tr> : records.map((record) => <tr key={record.id}><td>{String(record.data.date ?? "")}</td><td>{record.title}</td><td className="money-green">{money(Number(record.data.amount ?? 0))}</td><td>{String(record.data.note ?? "") || "—"}</td><td><div className="row-actions"><ActionButton onClick={() => begin(record)}>Sửa</ActionButton><ActionButton className="danger" onClick={() => removeRecord(record.id, reload)}>Xóa</ActionButton></div></td></tr>)}</tbody></table></div></div>{open && <div className="modal-backdrop"><ActionForm className="modal" onSubmit={save}><div className="modal-title"><h2>{editing ? "Cập nhật" : "Thêm mới"} {view.toLowerCase()}</h2><ActionButton type="button" onClick={() => setOpen(false)}>×</ActionButton></div><label>Nội dung<input required value={title} onChange={(e) => setTitle(e.target.value)}/></label><div className="form-grid two"><label>Ngày<input type="date" required value={date} onChange={(e) => setDate(e.target.value)}/></label><label>Số tiền / giá trị<input type="text" inputMode="numeric" pattern="[0-9,]*" value={amount} onChange={(e) => setAmount(formatVndInput(e.target.value))}/></label></div><label>Ghi chú<textarea value={note} onChange={(e) => setNote(e.target.value)}/></label>{message && <div className="form-message">{message}</div>}<div className="modal-actions"><ActionButton type="button" onClick={() => setOpen(false)}>Hủy</ActionButton><ActionButton className="primary-button">Lưu dữ liệu</ActionButton></div></ActionForm></div>}</>;
}
...
export function FunctionalTransfer({ stores }: { stores: FunctionalStore[] }) {
  ...
  return <div className="page-content"><div className="transfer-layout"><section className="form-card"><h2>1. Thông tin nhân viên</h2><label>Nhân viên<select value={employeeId} onChange={(e) => setEmployeeId(e.target.value)}>{employees.map((e) => <option key={e.id} value={e.id}>{e.code} · {e.name}</option>)}</select></label><label>Phụ cấp hỗ trợ<input type="text" inputMode="numeric" pattern="[0-9,]*" value={formatVndInput(allowance)} onChange={(e) => setAllowance(formatVndInput(e.target.value))}/></label></section><section className="form-card"><h2>2. Thông tin điều chuyển</h2><div className="form-grid two"><label>Cửa hàng nhận<select value={targetStore} onChange={(e) => setTargetStore(e.target.value)}>{stores.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label>Người phê duyệt<input value="Quản trị viên DORE" disabled/></label><label>Ngày bắt đầu<input type="date" value={start} onChange={(e) => setStart(e.target.value)}/></label><label>Ngày kết thúc<input type="date" value={end} onChange={(e) => setEnd(e.target.value)}/></label></div><label>Lý do<textarea value={reason} onChange={(e) => setReason(e.target.value)}/></label><ActionButton className="primary-button" onClick={save}>Lưu điều chuyển</ActionButton>{message && <div className={message.startsWith("✓") ? "success-banner" : "form-message"}>{message}</div>}</section><aside className="policy-card"><h2>3. Quyền truy cập</h2><p>✓ Kích hoạt quyền tại cửa hàng nhận khi được duyệt.</p><p>× Thu hồi quyền sau ngày kết thúc.</p><p>⌂ Bảo toàn lịch sử tại cửa hàng chính.</p></aside></div><div className="table-card"><div className="table-head"><h2>Lịch sử điều chuyển</h2><ActionButton onClick={() => downloadCsv("dieu-chuyen.csv", [["Nhân viên", "Cửa hàng", "Từ ngày", "Đến ngày", "Trạng thái"], ...records.map((r) => [String(r.data.employeeName ?? ""), String(r.data.targetName ?? ""), String(r.data.start ?? ""), String(r.data.end ?? ""), r.status])])}>Xuất Excel</ActionButton></div><table className="data-table"><thead><tr><th>Nhân viên</th><th>Cửa hàng hỗ trợ</th><th>Thời gian</th><th>Phụ cấp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td>{String(record.data.employeeName ?? "")}</td><td>{String(record.data.targetName ?? "")}</td><td>{String(record.data.start ?? "")} → {String(record.data.end ?? "")}</td><td>{money(Number(record.data.allowance ?? 0))}</td><td><span className="status-pill">{record.status}</span></td><td><div className="row-actions">{record.status === "PENDING" && <ActionButton onClick={() => setStatus(record, "APPROVED")}>Duyệt</ActionButton>}{!["CANCELLED", "COMPLETED"].includes(record.status) && <ActionButton onClick={() => setStatus(record, "COMPLETED")}>Kết thúc</ActionButton>}<ActionButton className="danger" onClick={() => setStatus(record, "CANCELLED")}>Hủy</ActionButton></div></td></tr>)}</tbody></table></div></div>;
}
...
export function FunctionalDividend({ totals }: { totals: { revenue: number; expense: number; profit: number } }) {
  ...
  return <div className="page-content"><div className="chart-grid dividend-grid"><section className="chart-card"><h2>Thông tin thành viên</h2><div className="shareholder"><span>PHẠM THỊ DIỄM THÚY <b>40%</b></span><strong>{money(thuy)}</strong></div><div className="shareholder"><span>TRƯƠNG VIỆT VI <b>60%</b></span><strong>{money(vi)}</strong></div><div className="share-total"><span>Tổng lợi nhuận được chia</span><b>{money(profit)}</b></div><ActionButton disabled={Boolean(existing)} className="primary-button wide" onClick={lock}>{existing ? "✓ Kỳ chia lợi nhuận đã khóa" : "Xác nhận chia lợi nhuận"}</ActionButton>{message && <div className="success-banner">{message}</div>}</section><section className="chart-card"><h2>Nguyên tắc tính</h2><p>Lợi nhuận sau cùng = Doanh thu − toàn bộ chi phí.</p><p>Số liệu được chụp tại thời điểm khóa kỳ và không tự thay đổi về sau.</p></section></div><div className="table-card"><div className="table-head"><h2>Lịch sử chia lợi nhuận</h2><ActionButton onClick={() => downloadCsv("chia-loi-nhuan.csv", [["Kỳ", "Doanh thu", "Chi phí", "Lợi nhuận", "Diễm Thúy", "Việt Vi"], ...records.map((r) => [String(r.data.month ?? ""), Number(r.data.revenue ?? 0), Number(r.data.expense ?? 0), Number(r.data.profit ?? 0), Number(r.data.thuy ?? 0), Number(r.data.vi ?? 0)])])}>Xuất Excel</ActionButton></div><table className="data-table"><thead><tr><th>Kỳ</th><th>Doanh thu</th><th>Chi phí</th><th>Lợi nhuận</th><th>Diễm Thúy</th><th>Việt Vi</th><th>Trạng thái</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td>{String(record.data.month ?? "")}</td><td>{money(Number(record.data.revenue ?? 0))}</td><td>{money(Number(record.data.expense ?? 0))}</td><td>{money(Number(record.data.profit ?? 0))}</td><td>{money(Number(record.data.thuy ?? 0))}</td><td>{money(Number(record.data.vi ?? 0))}</td><td><span className="status-pill">Đã khóa</span></td></tr>)}</tbody></table></div><div className="ai-analysis"><div className="analysis-illustration">↗</div><div><h2>📈 Kết luận phân tích kỳ {month}</h2><p>Lợi nhuận sau cùng đạt <b>{money(profit)}</b>, biên lợi nhuận <b>{totals.revenue ? (profit / totals.revenue * 100).toFixed(2) : "0"}%</b>. Thành viên Phạm Thị Diễm Thúy nhận {money(thuy)} và thành viên Trương Việt Vi nhận {money(vi)}.</p></div></div></div>;
}
...
export function FunctionalSettings({ name, email, storeId }: { name: string; email: string; storeId?: string | null }) {
  ...
  return <div className="page-content settings-layout">
    <aside className="settings-nav">
      <h2>Cài đặt</h2>
      <ActionButton type="button" className={activeTab === "profile" ? "active" : ""} onClick={() => setActiveTab("profile")}><UserRoundCog size={19}/><span>Thông tin cá nhân</span></ActionButton>
      <ActionButton type="button" className={activeTab === "password" ? "active" : ""} onClick={() => setActiveTab("password")}><KeyRound size={19}/><span>Đổi mật khẩu</span></ActionButton>
      <ActionButton type="button" disabled title="Tính năng đang được hoàn thiện"><BellRing size={19}/><span>Thông báo</span></ActionButton>
      <ActionButton type="button" disabled title="Hệ thống hiện dùng tiếng Việt"><Languages size={19}/><span>Ngôn ngữ</span></ActionButton>
    </aside>
    {activeTab === "profile" ? <section className="form-card settings-content">
      ...
      <ActionForm onSubmit={save}>
        ...
        <div className="settings-actions"><ActionButton type="submit" className="primary-button"><Save size={17}/> Lưu thay đổi</ActionButton></div>
      </ActionForm>
      ...
      <ActionForm className="password-settings-form" onSubmit={changePassword}>
        <label>Mật khẩu hiện tại<div className="settings-password-input"><LockKeyhole size={18}/><input required type={passwordVisible.current ? "text" : "password"} autoComplete="current-password" value={passwordForm.currentPassword} onChange={(event) => setPasswordForm({ ...passwordForm, currentPassword: event.target.value })}/><ActionButton type="button" aria-label={passwordVisible.current ? "Ẩn mật khẩu hiện tại" : "Hiện mật khẩu hiện tại"} onClick={() => togglePassword("current")}>{passwordVisible.current ? <EyeOff size={18}/> : <Eye size={18}/>}</ActionButton></div></label>
        <label>Mật khẩu mới<div className="settings-password-input"><KeyRound size={18}/><input required minLength={8} maxLength={128} type={passwordVisible.next ? "text" : "password"} autoComplete="new-password" value={passwordForm.newPassword} onChange={(event) => setPasswordForm({ ...passwordForm, newPassword: event.target.value })} aria-describedby="new-password-hint"/><ActionButton type="button" aria-label={passwordVisible.next ? "Ẩn mật khẩu mới" : "Hiện mật khẩu mới"} onClick={() => togglePassword("next")}>{passwordVisible.next ? <EyeOff size={18}/> : <Eye size={18}/>}</ActionButton></div><small id="new-password-hint">Ít nhất 8 ký tự và khác mật khẩu hiện tại.</small></label>
        <label>Xác nhận mật khẩu mới<div className="settings-password-input"><ShieldCheck size={18}/><input required minLength={8} maxLength={128} type={passwordVisible.confirm ? "text" : "password"} autoComplete="new-password" value={passwordForm.confirmPassword} onChange={(event) => setPasswordForm({ ...passwordForm, confirmPassword: event.target.value })}/><ActionButton type="button" aria-label={passwordVisible.confirm ? "Ẩn xác nhận mật khẩu" : "Hiện xác nhận mật khẩu"} onClick={() => togglePassword("confirm")}>{passwordVisible.confirm ? <EyeOff size={18}/> : <Eye size={18}/>}</ActionButton></div></label>
        <div className="settings-actions"><ActionButton type="submit" className="primary-button" disabled={passwordSaving}><KeyRound size={17}/> {passwordSaving ? "Đang đổi..." : "Đổi mật khẩu"}</ActionButton></div>
      </ActionForm>
