...
function shiftInfo(shift: ShiftRow) {
  ...
  return { ...shift, hourlyRate, hours, wage, total: wage + shift.tiktok_allowance };
}
...
export function ReferenceEmployeeShiftHistory() {
  ...
  return <div className="employee-reference history-reference"><div className="employee-history-filter"><label>Từ ngày<input type="date" value={from} onChange={(event) => setFrom(event.target.value)}/></label><label>Đến ngày<input type="date" value={to} onChange={(event) => setTo(event.target.value)}/></label><label>Ca làm<select value={query} onChange={(event) => setQuery(event.target.value)}><option value="">Tất cả</option><option value="Ca 1">Ca 1</option><option value="Ca 2">Ca 2</option><option value="Ca 3">Ca 3</option></select></label><button className="primary-button" onClick={reload}><Search size={17}/> Tìm kiếm</button></div>
    <section className="employee-panel table-panel"><div className="panel-title"><h2>LỊCH SỬ CA LÀM</h2><button onClick={() => csv("lich-su-ca-lam.csv", [["Ngày", "Ca làm", "Mã nhân viên", "Tên nhân viên", "Giờ vào", "Giờ kết ca", "Số giờ", "Lương giờ", "Lương dự tính"], ...rows.map((row) => [displayDay(workDay(row)), displayShiftName(row), row.employeeCode ?? "", row.employeeName ?? "", time(row.started_at), row.ended_at ? time(row.ended_at) : "Đang làm", row.hours.toFixed(2), row.hourlyRate, row.total])])}><Download size={16}/> Xuất Excel</button></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>STT</th><th>Thời gian (Ngày làm việc)</th><th>Mã nhân viên</th><th>Tên nhân viên</th><th>Ca làm</th><th>Thời gian vào</th><th>Thời gian kết ca</th><th>Số giờ làm</th><th>Lương cứng</th><th>Lương dự tính</th></tr></thead><tbody>{rows.length === 0 ? <tr><td className="empty-cell" colSpan={10}>Chưa có lịch sử ca làm trong thời gian đã chọn.</td></tr> : rows.map((row, index) => <tr key={row.id}><td>{index + 1}</td><td>{displayDay(workDay(row))}</td><td>{row.employeeCode ?? "—"}</td><td>{row.employeeName ?? "—"}</td><td><span className={`shift-pill s${shiftTone(row)}`}>{displayShiftName(row)}</span></td><td>{time(row.started_at)}</td><td>{row.ended_at ? time(row.ended_at) : "Đang làm"}</td><td className="money-green">{row.hours.toFixed(2)} giờ</td><td>{money(row.hourlyRate)}/giờ</td><td className="money-green">{money(row.total)}</td></tr>)}</tbody></table></div><div className="history-footer"><span>Hiển thị 1 - {rows.length} của {rows.length} bản ghi</span><div><button>‹</button><button className="active">1</button><button>›</button></div></div></section>
  </div>;
}
