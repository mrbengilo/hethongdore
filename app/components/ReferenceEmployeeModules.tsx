...
export function ReferenceEmployeePayroll() {
  ...
  return <div className="employee-reference payroll-reference"><div className="employee-filter"><label>Tháng<input type="month" value={month} onChange={(event) => setMonth(event.target.value)}/></label><label>Đến ngày<input type="date" value={through} onChange={(event) => setThrough(event.target.value)}/></label><button className="primary-button" onClick={reloadAll}><TrendingUp size={17}/> Xem thống kê</button></div>
    ...
    {sources.length > 0 && <section className="employee-panel table-panel"><div className="panel-title"><h2>NGUỒN CHI TRẢ THEO CỬA HÀNG</h2><span>{sources.length} cửa hàng</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Cửa hàng</th><th>Vai trò</th><th>Giờ thực tế</th><th>Lương cứng</th><th>Phụ cấp hỗ trợ</th><th>Thưởng/KPI</th><th>Thực nhận</th><th>Trạng thái</th></tr></thead><tbody>{sources.map((source) => <tr key={source.storeId}><td><b>{source.storeName}</b></td><td>{source.isSupport ? <span className="status-pill">Nhân viên hỗ trợ</span> : "Nhân viên chính"}</td><td>{source.hours.toFixed(2)} giờ</td><td>{money(source.baseSalary)}</td><td>{money(source.supportAllowance)}</td><td>{money(source.manualBonus + source.kpiBonus + source.manualAllowance)}</td><td className="money-green"><b>{money(source.totalPay)}</b></td><td><span className="status-pill">{source.paymentStatus === "LOCKED" || source.paymentStatus === "PAYMENT_CONFIRMED" ? "Đã chi" : "Chờ chi"}</span></td></tr>)}</tbody></table></div></section>}
    <div className="employee-payroll-grid"><section className="employee-panel table-panel"><div className="panel-title"><h2>CHI TIẾT LƯƠNG THEO CA · NHÂN VIÊN CHÍNH / HỖ TRỢ</h2><button onClick={() => csv("bang-luong.csv", [["Ngày", "Ca", "Cửa hàng", "Vai trò", "Giờ vào", "Giờ kết", "Giờ thực tế", "Lương/giờ", "Lương cứng", "Phụ cấp hỗ trợ", "Phụ cấp TikTok", "Thực nhận"], ...detailRows.map((row) => [displayDay(row.workDate || localDay(row.startedAt)), row.shiftName ?? row.shiftCode, row.storeName, row.isSupport ? "Nhân viên hỗ trợ" : "Nhân viên chính", time(row.startedAt), time(row.endedAt), row.hours.toFixed(2), row.hourlyRate, row.baseSalary, row.supportAllowance, row.tiktokAllowance, row.netPay])])}><Download size={16}/> Xuất Excel</button></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Ngày</th><th>Ca / cửa hàng</th><th>Vai trò</th><th>Giờ thực tế</th><th>Lương hỗ trợ/giờ</th><th>Lương cứng</th><th>Phụ cấp hỗ trợ</th><th>Thực nhận ca</th></tr></thead><tbody>{detailRows.length === 0 ? <tr><td className="empty-cell" colSpan={8}>Chưa có ca làm trong thời gian đã chọn.</td></tr> : detailRows.map((row) => <tr key={row.id}><td>{displayDay(row.workDate || localDay(row.startedAt))}<small>{time(row.startedAt)} - {time(row.endedAt)}</small></td><td><b>{row.shiftName ?? row.shiftCode}</b><small>{row.storeName}</small></td><td>{row.isSupport ? <><span className="status-pill">Nhân viên hỗ trợ</span><small className="support-note">Từ {row.sourceStoreName ?? "cửa hàng chính"}</small></> : <span>Nhân viên chính</span>}</td><td>{row.hours.toFixed(2)} giờ</td><td>{money(row.hourlyRate)}/giờ</td><td>{money(row.baseSalary)}</td><td>{row.isSupport ? money(row.supportAllowance) : "—"}</td><td className="money-green"><b>{money(row.netPay)}</b></td></tr>)}</tbody></table></div></section><aside className="employee-panel income-summary"><h2>TỔNG KẾT THU NHẬP</h2><p><span>Tổng lương ({rowCount} ca)</span><b>{money(wage)}</b></p><p><span>Tổng phụ cấp</span><b>{money(allowance)}</b></p><p><span>Thưởng khác</span><b>{money(payrollItem?.manualBonus ?? 0)}</b></p><p><span>Thưởng KPI</span><b>{money(payrollItem?.kpiBonus ?? 0)}</b></p><p className="total"><span>TỔNG THỰC NHẬN</span><b>{money(income)}</b></p><div><BadgeDollarSign size={30}/><span>{payrollPaid ? "Đã chi lương, thưởng và phụ cấp" : payrollLocked ? "Đã chốt, đang chờ chi" : "KPI chỉ hiển thị sau tổng kết"}<br/><b>{month}</b></span></div></aside></div>
  </div>;
}
