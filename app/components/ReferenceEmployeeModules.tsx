...
export function ReferenceEmployeeShiftHistory() {
  ...
  return <div className="employee-reference history-reference"><div className="employee-history-filter"><label>Từ ngày<input type="date" value={from} onChange={(event) => setFrom(event.target.value)}/></label><label>Đến ngày<input type="date" value={to} onChange={(event) => setTo(event.target.value)}/></label><label>Ca làm<select value={query} onChange={(event) => setQuery(event.target.value)}><option value="">Tất cả</option><option value="Ca 1">Ca 1</option><option value="Ca 2">Ca 2</option><option value="Ca 3">Ca 3</option></select></label><ActionButton className="primary-button" onClick={reload}><Search size={17}/> Tìm kiếm</ActionButton></div>
    <section className="employee-panel table-panel"><div className="panel-title"><h2>LỊCH SỬ CA LÀM</h2><ActionButton onClick={() => csv("lich-su-ca-lam.csv", [["Ngày", "Ca làm", "Mã nhân viên", "Tên nhân viên", "Giờ vào", "Giờ kết ca", "Số giờ", "Lương giờ", "Phụ cấp hỗ trợ", "Lương dự tính"], ...rows.map((row) => [displayDay(workDay(row)), displayShiftName(row), row.employeeCode ?? "", row.employeeName ?? "", time(row.started_at), row.ended_at ? time(row.ended_at) : "Đang làm", row.hours.toFixed(2), row.hourlyRate, row.supportAllowance, row.total])])}><Download size={16}/> Xuất Excel</ActionButton></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>STT</th><th>Ngày làm việc</th><th>Mã nhân viên</th><th>Tên nhân viên</th><th>Ca làm</th><th>Giờ vào</th><th>Giờ kết ca</th><th>Số giờ</th><th>Lương/giờ</th><th>Phụ cấp hỗ trợ</th><th>Lương dự tính</th></tr></thead><tbody>{rows.length === 0 ? <tr><td className="empty-cell" colSpan={11}>Chưa có lịch sử ca làm trong thời gian đã chọn.</td></tr> : rows.map((row, index) => <tr key={row.id}><td>{index + 1}</td><td>{displayDay(workDay(row))}</td><td>{row.employeeCode ?? "—"}</td><td><b>{row.employeeName ?? "—"}</b><SupportTag supporting={Boolean(row.transfer_id)} sourceStoreName={row.sourceStoreName}/></td><td><span className={`shift-pill s${shiftTone(row)}`}>{displayShiftName(row)}</span>{row.transfer_id && <small className="support-note">CA HỖ TRỢ · {row.targetStoreName ?? "Cửa hàng hỗ trợ"}</small>}</td><td>{time(row.started_at)}</td><td>{row.ended_at ? time(row.ended_at) : "Đang làm"}</td><td className="money-green">{row.hours.toFixed(2)} giờ</td><td>{money(row.hourlyRate)}/giờ</td><td>{row.transfer_id ? money(row.supportAllowance) : "—"}</td><td className="money-green">{money(row.total)}</td></tr>)}</tbody></table></div><div className="history-footer"><span>Hiển thị 1 - {rows.length} của {rows.length} bản ghi</span><div><ActionButton>‹</ActionButton><ActionButton className="active">1</ActionButton><ActionButton>›</ActionButton></div></div></section>
  </div>;
}
