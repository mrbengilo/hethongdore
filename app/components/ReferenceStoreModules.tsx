...
export function ReferenceEmployees({ store }: { store: ReferenceStore }) {
  ...
  async function remove(employee: Employee) {
    if (!confirm("Xóa nhân viên " + employee.name + "?")) return;
  ...
  return <div className="reference-module">
    <div className="ref-toolbar"><div><h2>Quản lý nhân viên</h2><p>Thêm, sửa, xóa và quản lý tài khoản nhân viên</p></div><div className="ref-toolbar-actions"><label className="ref-search"><Search size={16}/><input placeholder="Tìm kiếm nhân viên..." value={query} onChange={(e) => setQuery(e.target.value)}/></label><button className="primary-button" onClick={() => begin()}><Plus size={17}/> Thêm nhân viên</button></div></div>
    <div className="ref-metrics four"><Metric icon={UsersRound} label="Tổng nhân viên" value={String(employees.length)} note="Tất cả nhân viên"/><Metric icon={UserRound} label="Đang làm việc" value={String(employees.filter(e => e.status === "ACTIVE").length)} note="100%"/><Metric icon={Clock3} label="Tạm nghỉ" value="0" note="0%" tone="orange"/><Metric icon={UserRound} label="Đã nghỉ việc" value="0" note="0%" tone="red"/></div>
    <div className={"employee-ref-layout " + (open ? "with-drawer" : "")}><section className="table-card">
      <div className="ref-tabs"><button className={tab === "ALL" ? "active" : ""} onClick={() => setTab("ALL")}>Tất cả ({employees.length})</button><button className={tab === "ACTIVE" ? "active" : ""} onClick={() => setTab("ACTIVE")}>Đang làm việc ({employees.length})</button><button onClick={() => setTab("BREAK")}>Tạm nghỉ (0)</button><button onClick={() => setTab("ARCHIVED")}>Đã nghỉ việc (0)</button></div>
      <div className="data-table-wrap"><table className="data-table ref-employee-table"><thead><tr><th>Mã nhân viên</th><th>Họ và tên</th><th>SĐT</th><th>Chức vụ</th><th>Tên đăng nhập</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{filtered.length ? filtered.map((employee) => <tr key={employee.id}><td><b>{employee.code}</b></td><td><Person name={employee.name} position={employee.position}/></td><td>{employee.phone}</td><td>{employee.position}</td><td>{employee.username ?? "—"}</td><td><span className="status-pill">● Đang làm việc</span></td><td><div className="row-actions"><button onClick={() => begin(employee)} title="Sửa"><Edit3 size={15}/></button><button className="danger" onClick={() => remove(employee)} title="Xóa"><Trash2 size={15}/></button></div></td></tr>) : <tr><td colSpan={7} className="empty-cell">Không có nhân viên phù hợp.</td></tr>}</tbody></table></div>
      ...
      {message && <div className="form-message">{message}</div>}<div className="drawer-actions"><button type="button" onClick={() => setOpen(false)}>Hủy bỏ</button><button className="primary-button">Lưu nhân viên</button></div>
    </form></aside>}</div>
  </div>;
}
...
export function ReferenceStoreModule({ store, view }: { store: ReferenceStore; view: string }) {
  ...
  if (view === "Lương thưởng") return <PayrollManagement store={store}/>;
...
function ScheduleManagement({ store }: { store: ReferenceStore }) {
  ...
  const shifts = shiftRecords.length ? shiftRecords.map((r,index)=>({id:r.id,title:r.title,start:String(r.data.start??"07:00"),end:String(r.data.end??"15:00"),tone:"s"+((index%3)+1)})) : defaultShifts;
...
function GoodsManagement({ store }: { store: ReferenceStore }) {
  ...
  const samples=sampleGoods.map((g,index)=>({id:"sample-"+index,title:g[0],data:{quantity:g[1],unit:g[2],weight:g[3],unitPrice:g[4],shipping:g[5],amount:g[3]*g[4]+g[5],date:"2025-05-"+String(15-index).padStart(2,"0"),note:""}}));
  const rows=(records.length?records:samples).filter(r=>r.title.toLocaleLowerCase("vi").includes(query.toLocaleLowerCase("vi")));
  ...
  async function remove(record:BusinessRecord){if(record.id.startsWith("sample-"))return; if(await deleteRecord(record.id))await reload();}
...
function AttendanceManagement({ store }: { store: ReferenceStore }) {
  const { shifts }=useShiftSessions(store.id); const [tab,setTab]=useState<"day"|"shift">("day"); const [query,setQuery]=useState(""); const [date,setDate]=useState(today());
  const actual=shifts.filter(s=>s.employeeName.toLocaleLowerCase("vi").includes(query.toLocaleLowerCase("vi")));
  const fallback=samplePeople.map((p,i)=>({id:"sample-"+i,employeeName:p[0],employeeCode:"NV00"+(i+1),started_at:"2025-05-15T"+["06:58","11:59","16:58","12:03","07:02","17:10"][i]+":00+07:00",ended_at:"2025-05-15T"+["12:05","17:02","23:05","17:00","12:00","22:55"][i]+":00+07:00",hourlyRate:20000,status:"COMPLETED",shift_code:"Ca "+((i%3)+1)})); const rows=actual.length?actual:fallback;
  const totalHours=rows.reduce((sum,row)=>sum+Math.max(0,(new Date(row.ended_at??new Date()).getTime()-new Date(row.started_at).getTime())/3600000),0); const totalPay=rows.reduce((sum,row)=>sum+Math.round(Math.max(0,(new Date(row.ended_at??new Date()).getTime()-new Date(row.started_at).getTime())/3600000)*row.hourlyRate),0);
  return <div className="reference-module attendance-page"><div className="ref-toolbar"><div><h2>Chấm công</h2><p>Theo dõi giờ làm việc của nhân viên</p></div><div className="ref-toolbar-actions"><input type="date" value={date} onChange={(e)=>setDate(e.target.value)}/><button><ChevronLeft size={16}/></button><button><ChevronRight size={16}/></button><button onClick={()=>csv("cham-cong.csv",[["Nhân viên","Mã ca","Giờ vào","Giờ kết","Lương"],...rows.map(r=>[r.employeeName,r.shift_code,r.started_at,r.ended_at,Math.round(((new Date(r.ended_at??new Date()).getTime()-new Date(r.started_at).getTime())/3600000)*r.hourlyRate)])])}><Download size={16}/> Xuất Excel</button></div></div>
    <div className="ref-metrics four"><Metric icon={UsersRound} label="Tổng số nhân viên" value={rows.length+" người"}/><Metric icon={Clock3} label="Tổng giờ làm" value={totalHours.toFixed(2)+" giờ"} note={"TB: "+(totalHours/Math.max(1,rows.length)).toFixed(2)+" giờ/người"}/><Metric icon={WalletCards} label="Tổng lương cứng" value={money(totalPay)} note="Theo lương giờ"/><Metric icon={WalletCards} label="Tổng lương nhận" value={money(totalPay)}/></div>
    <section className="table-card"><div className="table-head"><div className="ref-tabs compact"><button className={tab==="day"?"active":""} onClick={()=>setTab("day")}>Tổng hợp theo ngày</button><button className={tab==="shift"?"active":""} onClick={()=>setTab("shift")}>Chi tiết theo ca</button></div><label className="ref-search"><Search size={16}/><input placeholder="Tìm kiếm nhân viên..." value={query} onChange={(e)=>setQuery(e.target.value)}/></label></div><div className="data-table-wrap"><table className="data-table attendance-table"><thead><tr><th>STT</th><th>Nhân viên</th><th>Ca làm việc</th><th>Giờ vào</th><th>Giờ kết ca</th><th>Số giờ làm</th><th>Lương cứng</th><th>Lương nhận</th><th>Trạng thái</th></tr></thead><tbody>{rows.map((row,index)=>{const hours=Math.max(0,(new Date(row.ended_at??new Date()).getTime()-new Date(row.started_at).getTime())/3600000);const pay=Math.round(hours*row.hourlyRate);const late=new Date(row.started_at).getMinutes()>5;return <tr key={row.id}><td>{index+1}</td><td><Person name={row.employeeName} position={row.employeeCode}/></td><td><b className={"shift-text c"+((index%3)+1)}>{row.shift_code}</b></td><td className={late?"money-orange":"money-green"}>{timeLabel(row.started_at)}</td><td>{timeLabel(row.ended_at)}</td><td>{hours.toFixed(2)} giờ</td><td>{money(pay)}</td><td className="money-green">{money(pay)}</td><td><span className={"status-pill "+(late?"late":"")}>{late?"Trễ":"Đúng giờ"}</span></td></tr>})}</tbody></table></div></section>
    <div className="ref-bottom-grid"><article className="chart-card"><h3>Giải thích</h3><p>• Số giờ làm = Giờ kết ca − Giờ vào</p><p>• Lương cứng = Số giờ làm × lương theo giờ</p></article><article className="chart-card"><h3>Tổng hợp theo ca</h3><div className="shift-info"><span><b>Ca 1</b>{(totalHours*.36).toFixed(2)} giờ</span><span><b>Ca 2</b>{(totalHours*.35).toFixed(2)} giờ</span><span><b>Ca 3</b>{(totalHours*.29).toFixed(2)} giờ</span></div></article><article className="chart-card donut-small"><div className="ref-donut attendance"><b>{rows.length}</b><small>tổng số</small></div><div><b>Tỷ lệ chấm công</b><p>Đúng giờ: 83,33%</p><p>Trễ: 16,67%</p></div></article></div>
  </div>;
}

function PayrollManagement({ store }: { store: ReferenceStore }) {
  const {records,reload}=useRecords("LUONG_THUONG",store.id); const {employees}=useEmployees(store.id); const [period,setPeriod]=useState<"day"|"month">("month"); const [kind,setKind]=useState<"ALLOWANCE"|"BONUS">("ALLOWANCE"); const [open,setOpen]=useState(false); const [employeeId,setEmployeeId]=useState(""); const [amount,setAmount]=useState(""); const [note,setNote]=useState(""); const [date,setDate]=useState(today()); const [message,setMessage]=useState("");
  useEffect(()=>{if(!employeeId&&employees[0])setEmployeeId(employees[0].id)},[employeeId,employees]);
  const allowances=records.filter(r=>r.data.kind==="ALLOWANCE"); const bonuses=records.filter(r=>r.data.kind==="BONUS"); const allowanceTotal=allowances.reduce((s,r)=>s+Number(r.data.amount??0),0); const bonusTotal=bonuses.reduce((s,r)=>s+Number(r.data.amount??0),0); const profitBonus=Math.max(0,store.profit*.05);
  function begin(type:"ALLOWANCE"|"BONUS"){setKind(type);setAmount("");setNote("");setMessage("");setOpen(true);}
  async function save(event:FormEvent){event.preventDefault();const employee=employees.find(e=>e.id===employeeId);if(!employee||Number(amount)<=0)return setMessage("Vui lòng chọn nhân viên và nhập số tiền hợp lệ.");try{await saveRecord({category:"LUONG_THUONG",storeId:store.id,title:(kind==="ALLOWANCE"?"Phụ cấp":"Thưởng")+" · "+employee.name,data:{kind,employeeId,employeeName:employee.name,amount:Number(amount),note,date}});setOpen(false);await reload();}catch(error){setMessage((error as Error).message);}}
  async function remove(id:string){if(await deleteRecord(id))await reload();}
  const baseTotal=employees.reduce((sum,e)=>sum+e.hourly_rate*200,0); const total=baseTotal+allowanceTotal+bonusTotal+profitBonus;
  return <div className="reference-module payroll-page"><div className="ref-toolbar"><div><h2>Lương thưởng nhân viên</h2><p>Quản lý và tổng hợp lương, thưởng, phụ cấp theo nhân viên</p></div><div className="ref-toolbar-actions"><div className="ref-tabs compact"><button className={period==="day"?"active":""} onClick={()=>setPeriod("day")}>Ngày</button><button className={period==="month"?"active":""} onClick={()=>setPeriod("month")}>Tháng</button></div><input type={period==="month"?"month":"date"} defaultValue={period==="month"?today().slice(0,7):today()}/><button onClick={()=>csv("luong-thuong.csv",[["Ngày","Nhân viên","Loại","Số tiền","Nội dung"],...records.map(r=>[String(r.data.date),String(r.data.employeeName),String(r.data.kind),Number(r.data.amount),String(r.data.note)])])}><Download size={16}/> Xuất Excel</button><button onClick={()=>begin("ALLOWANCE")}><Plus size={16}/> Tạo phụ cấp</button><button className="primary-button" onClick={()=>begin("BONUS")}><Gift size={16}/> Tạo thưởng</button></div></div>
    <div className="ref-metrics six"><Metric icon={Clock3} label="Tổng giờ làm" value={(employees.length*200).toLocaleString("vi-VN")+" giờ"}/><Metric icon={WalletCards} label="Tổng lương" value={money(baseTotal)}/><Metric icon={Gift} label="Tổng thưởng" value={money(bonusTotal+profitBonus)} tone="orange"/><Metric icon={WalletCards} label="Tổng phụ cấp" value={money(allowanceTotal)}/><Metric icon={UsersRound} label="Tổng chi trả" value={money(total)} tone="teal"/><Metric icon={UserRound} label="Tổng nhân viên" value={employees.length+" nhân viên"}/></div>
    <section className="table-card"><div className="table-head"><h2>Chi tiết lương thưởng nhân viên</h2><span>{period==="month"?"Tổng kết theo tháng":"Theo ngày đang chọn"}</span></div><div className="data-table-wrap"><table className="data-table payroll-table"><thead><tr><th>Nhân viên</th><th>Tổng giờ làm</th><th>Lương cứng</th><th>Phụ cấp</th><th>Thưởng đã tạo</th><th>Thưởng lợi nhuận</th><th>Tổng nhận</th></tr></thead><tbody>{employees.map((employee)=>{const base=employee.hourly_rate*200;const empAllowance=allowances.filter(r=>r.data.employeeId===employee.id).reduce((s,r)=>s+Number(r.data.amount),0);const empBonus=bonuses.filter(r=>r.data.employeeId===employee.id).reduce((s,r)=>s+Number(r.data.amount),0);const share=employees.length?Math.round(profitBonus/employees.length):0;return <tr key={employee.id}><td><Person name={employee.name} position={employee.position}/></td><td>200,00 giờ</td><td>{money(base)}</td><td className="money-green">{money(empAllowance)}</td><td>{money(empBonus)}</td><td className="money-green">{money(share)}</td><td><b>{money(base+empAllowance+empBonus+share)}</b></td></tr>})}</tbody></table></div></section>
    <section className="table-card"><div className="table-head"><h2>Lịch sử tạo phụ cấp và thưởng</h2><span>{records.length} bản ghi</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Thời gian</th><th>Nhân viên</th><th>Loại</th><th>Số tiền</th><th>Nội dung chi</th><th>Người tạo</th><th>Thao tác</th></tr></thead><tbody>{records.length?records.map(record=><tr key={record.id}><td>{String(record.data.date)}</td><td><b>{String(record.data.employeeName)}</b></td><td><span className={record.data.kind==="BONUS"?"bonus-pill":"allowance-pill"}>{record.data.kind==="BONUS"?"Thưởng":"Phụ cấp"}</span></td><td className="money-green"><b>{money(Number(record.data.amount))}</b></td><td>{String(record.data.note||"—")}</td><td>Quản lý cửa hàng</td><td><button className="danger-link" onClick={()=>remove(record.id)}>Xóa</button></td></tr>):<tr><td colSpan={7} className="empty-cell">Chưa phát sinh phụ cấp hoặc thưởng trong kỳ.</td></tr>}</tbody></table></div></section>
    <div className="ref-chart-row"><article className="chart-card donut-small"><div className="ref-donut payroll"><b>{money(total)}</b><small>Tổng chi trả</small></div><div><b>Cơ cấu chi trả</b><p>Lương cứng</p><p>Thưởng và phụ cấp</p></div></article><article className="chart-card"><h3>Thống kê giờ làm theo ngày</h3><MiniBars/></article><article className="chart-card quick-total"><h3>Tóm tắt nhanh</h3><p><span>Lương cứng</span><b>{money(baseTotal)}</b></p><p><span>Phụ cấp</span><b>{money(allowanceTotal)}</b></p><p><span>Thưởng</span><b>{money(bonusTotal+profitBonus)}</b></p></article></div>
    {open&&<div className="modal-backdrop"><form className="modal payroll-action-modal" onSubmit={save}><div className="modal-title"><div><h2>{kind==="ALLOWANCE"?"Tạo phụ cấp":"Tạo thưởng"}</h2><p>Khoản chi được ghi nhận đúng nhân viên và cửa hàng</p></div><button type="button" onClick={()=>setOpen(false)}><X size={19}/></button></div><label>Nhân viên được nhận *<select value={employeeId} onChange={(e)=>setEmployeeId(e.target.value)}>{employees.map(employee=><option key={employee.id} value={employee.id}>{employee.code} · {employee.name}</option>)}</select></label><label>Số tiền {kind==="ALLOWANCE"?"phụ cấp":"thưởng"} *<input type="number" min="1" required value={amount} onChange={(e)=>setAmount(e.target.value)}/></label><label>Ngày ghi nhận<input type="date" value={date} onChange={(e)=>setDate(e.target.value)}/></label><label>Nội dung chi *<textarea required value={note} onChange={(e)=>setNote(e.target.value)} placeholder={kind==="ALLOWANCE"?"Ví dụ: Phụ cấp chuyên cần":"Ví dụ: Thưởng hoàn thành KPI"}/></label>{message&&<div className="form-message">{message}</div>}<div className="modal-actions"><button type="button" onClick={()=>setOpen(false)}>Hủy</button><button className="primary-button">Lưu {kind==="ALLOWANCE"?"phụ cấp":"thưởng"}</button></div></form></div>}
  </div>;
}
...
function ReportManagement({ store }: { store: ReferenceStore }) {
  const {employees}=useEmployees(store.id); const {shifts}=useShiftSessions(store.id); const payroll=useRecords("LUONG_THUONG",store.id).records; const [tab,setTab]=useState("Tổng quan"); const [from,setFrom]=useState(today().slice(0,8)+"01"); const [to,setTo]=useState(today());
  const completed=shifts.filter(s=>s.ended_at); const hours=completed.reduce((sum,s)=>sum+(new Date(s.ended_at!).getTime()-new Date(s.started_at).getTime())/3600000,0); const wages=completed.reduce((sum,s)=>sum+((new Date(s.ended_at!).getTime()-new Date(s.started_at).getTime())/3600000)*s.hourlyRate,0); const extras=payroll.reduce((sum,r)=>sum+Number(r.data.amount??0),0);
