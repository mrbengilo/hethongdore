...
function Person({ name, position, isSupport, sourceStoreName }: { name: string; position: string; isSupport?: boolean | number; sourceStoreName?: string | null }) {
  return <div className="ref-person"><i>{name.slice(0, 1)}</i><span><b>{name}</b><SupportTag supporting={isSupport} sourceStoreName={sourceStoreName}/><small>{position}</small></span></div>;
}
...
export function ReferenceEmployees({ store }: { store: ReferenceStore }) {
  ...
  return <div className="reference-module">
    <div className="ref-toolbar"><div><h2>Quản lý nhân viên</h2><p>Thêm, sửa thông tin và cập nhật trạng thái làm việc</p></div><div className="ref-toolbar-actions"><label className="ref-search"><Search size={16}/><input placeholder="Tìm kiếm nhân viên..." value={query} onChange={(e) => setQuery(e.target.value)}/></label><ActionButton className="primary-button" onClick={() => begin()}><Plus size={17}/> Thêm nhân viên</ActionButton></div></div>
    ...
    <div className={"employee-ref-layout " + (open ? "with-drawer" : "")}><section className="table-card">
      <div className="ref-tabs"><ActionButton className={tab === "ALL" ? "active" : ""} onClick={() => setTab("ALL")}>Tất cả ({employees.length})</ActionButton><ActionButton className={tab === "ACTIVE" ? "active" : ""} onClick={() => setTab("ACTIVE")}>Đang làm việc ({employees.filter(e => e.status === "ACTIVE").length})</ActionButton><ActionButton className={tab === "INACTIVE" ? "active" : ""} onClick={() => setTab("INACTIVE")}>Nghỉ làm ({employees.filter(e => e.status === "INACTIVE").length})</ActionButton></div>
      <div className="data-table-wrap"><table className="data-table ref-employee-table"><thead><tr><th>Mã nhân viên</th><th>Họ và tên</th><th>SĐT</th><th>Chức vụ</th><th>Tên đăng nhập</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{filtered.length ? filtered.map((employee) => <tr key={employee.id}><td><b>{employee.code}</b></td><td><Person isSupport={employee.isSupport} sourceStoreName={employee.homeStoreName} name={employee.name} position={employee.position}/></td><td>{employee.phone}</td><td>{employee.position}</td><td>{employee.username ?? "—"}</td><td><span className={`status-pill ${employee.status === "INACTIVE" ? "inactive" : ""}`}>● {employee.status === "INACTIVE" ? "Nghỉ làm" : "Đang làm việc"}</span></td><td><div className="row-actions"><ActionButton onClick={() => begin(employee)} title="Sửa nhân viên"><Edit3 size={15}/></ActionButton></div></td></tr>) : <tr><td colSpan={7} className="empty-cell">Không có nhân viên phù hợp.</td></tr>}</tbody></table></div>
    </section>{open && <aside className="employee-drawer"><ActionForm onSubmit={save}><div className="drawer-title"><div><h2>{editing ? "Cập nhật nhân viên" : "Thêm nhân viên"}</h2><span>Thông tin nhân viên</span></div><ActionButton type="button" onClick={() => setOpen(false)}><X size={19}/></ActionButton></div>
      ...
      {message && <div className="form-message">{message}</div>}<div className="drawer-actions"><ActionButton type="button" onClick={() => setOpen(false)} disabled={saving}>Hủy bỏ</ActionButton><ActionButton type="submit" className="primary-button" disabled={saving}>{saving ? "Đang lưu..." : "Lưu nhân viên"}</ActionButton></div>
    </ActionForm></aside>}</div>
  </div>;
}
...
function ShiftManagement({ store }: { store: ReferenceStore }) {
  ...
  return <div className="reference-module">
    <div className="ref-toolbar"><div><h2>Ca làm việc</h2><p>Quản lý tên ca và thời gian làm việc</p></div><div className="ref-toolbar-actions"><input type="date" value={date} onChange={(e) => setDate(e.target.value)}/><ActionButton onClick={() => csv("ca-lam-viec.csv", [["Tên ca","Bắt đầu","Kết thúc"], ...shifts.map(s => [s.title,s.start,s.end])])}><Download size={16}/> Xuất Excel</ActionButton><ActionButton className="primary-button" onClick={() => begin()}><Plus size={17}/> Tạo ca làm việc</ActionButton></div></div>
    <div className="ref-shift-grid">{shifts.length ? shifts.slice(0, 3).map((shift, index) => <article className={"ref-shift " + ["green","orange","purple"][index % 3]} key={shift.id}><span>{shift.title}</span><strong>{shift.start && shift.end ? `${shift.start} - ${shift.end}` : "Chưa đặt thời gian"}</strong><small><UsersRound size={15}/> {assignmentCount(shift.id, date)} nhân viên đã xếp</small>{shift.record && <div className="shift-card-actions"><ActionButton onClick={() => begin(shift.record)}><Edit3 size={14}/></ActionButton><ActionButton onClick={() => remove(shift.record!)}><Trash2 size={14}/></ActionButton></div>}</article>) : <p className="empty-cell" role="status">Chưa có ca làm việc. Hãy tạo ca trước khi phân lịch.</p>}<article className="ref-day-summary"><b>Tổng quan ngày {dateLabel(date)}</b><div><span><strong>{shifts.length}</strong> Tổng ca</span><span><strong>{employees.length}</strong> Nhân viên</span><span><strong>{dayAssignmentCount}</strong> Lượt phân ca</span></div></article></div>
    <section className="table-card"><div className="table-head"><div><h2>{mode === "day" ? "Lịch phân ca ngày " + dateLabel(date) : "Lịch làm việc trong tuần"}</h2><p>Chọn chế độ xem ngày hoặc tuần</p></div><div className="ref-tabs compact"><ActionButton className={mode === "day" ? "active" : ""} onClick={() => setMode("day")}>Lịch theo ngày</ActionButton><ActionButton className={mode === "week" ? "active" : ""} onClick={() => setMode("week")}>Lịch theo tuần</ActionButton></div></div>
    ...
    {open && <div className="modal-backdrop"><ActionForm className="modal shift-definition-modal" onSubmit={save}><div className="modal-title"><div><h2>{editing ? "Cập nhật ca làm việc" : "Thêm ca làm việc"}</h2><p>Chỉ cần nhập tên ca và thời gian áp dụng</p></div><ActionButton type="button" onClick={() => setOpen(false)}><X size={19}/></ActionButton></div><label>Tên ca *<input required placeholder="Ví dụ: Ca sáng" value={name} onChange={(e) => setName(e.target.value)}/></label><div className="form-grid two"><label>Thời gian bắt đầu *<input type="time" required value={start} onChange={(e) => setStart(e.target.value)}/></label><label>Thời gian kết thúc *<input type="time" required value={end} onChange={(e) => setEnd(e.target.value)}/></label></div>{message && <div className="form-message">{message}</div>}<div className="modal-actions"><ActionButton type="button" onClick={() => setOpen(false)}>Hủy</ActionButton><ActionButton className="primary-button">Lưu ca làm việc</ActionButton></div></ActionForm></div>}
  </div>;
}

type SchedulePerson = { id: string; name: string; position: string; isSupport?: number; homeStoreName?: string | null };

function schedulePeople(employees: Employee[], records: BusinessRecord[]): SchedulePerson[] {
  const people = new Map<string, SchedulePerson>(employees.map((employee) => [employee.id, { id: employee.id, name: employee.name, position: employee.position, isSupport: employee.isSupport, homeStoreName: employee.homeStoreName }]));
...
function ScheduleManagement({ store }: { store: ReferenceStore }) {
  ...
  return <div className="reference-module schedule-page">
    <div className="ref-toolbar"><div><h2>Lịch phân ca</h2><p>Tạo và quản lý lịch phân công ca làm việc cho nhân viên</p></div><div className="ref-toolbar-actions"><ActionButton onClick={()=>setDate(new Date(new Date(date).getTime()-86400000).toISOString().slice(0,10))}><ChevronLeft size={17}/></ActionButton><input type="date" value={date} onChange={(e)=>setDate(e.target.value)}/><ActionButton onClick={()=>setDate(new Date(new Date(date).getTime()+86400000).toISOString().slice(0,10))}><ChevronRight size={17}/></ActionButton><ActionButton onClick={()=>csv("lich-phan-ca.csv",[["Ngày","Ca","Nhân viên"],...records.map(r=>[String(r.data.date??""),String(r.data.shiftName??""),String((r.data.employeeNames as string[]|undefined)?.join(", ")??"")])])}><Download size={16}/> Xuất Excel</ActionButton><ActionButton className="primary-button" disabled={!shifts.length || !employees.length} title={!shifts.length ? "Hãy tạo ca làm việc trước" : !employees.length ? "Chưa có nhân viên để phân ca" : undefined} onClick={()=>begin()}><Plus size={17}/> Tạo lịch phân ca</ActionButton></div></div>
...
function GoodsManagement({ store }: { store: ReferenceStore }) {
  ...
  return <div className="reference-module goods-page"><div className="ref-toolbar"><div><h2>Nhập hàng</h2><p>Quản lý danh sách mặt hàng nhập kho</p></div><div className="ref-toolbar-actions"><ActionButton onClick={()=>csv("nhap-hang.csv",[["Mặt hàng","Số lượng","Cân nặng","Đơn giá","Thành tiền"],...rows.map(r=>[r.title,Number(r.data.quantity),Number(r.data.weight),Number(r.data.unitPrice),Number(r.data.amount)])])}><Download size={16}/> Xuất Excel</ActionButton><ActionButton className="primary-button" onClick={()=>begin()}><Plus size={17}/> Thêm mặt hàng</ActionButton></div></div>
    ...
    <section className="table-card"><div className="table-head"><label className="ref-search"><Search size={16}/><input placeholder="Tìm kiếm mặt hàng..." value={query} onChange={(e)=>setQuery(e.target.value)}/></label><ActionButton>Tất cả danh mục</ActionButton></div><div className="data-table-wrap"><table className="data-table goods-table"><thead><tr><th>STT</th><th>Tên mặt hàng</th><th>Số lượng</th><th>Đơn vị</th><th>Cân nặng (kg)</th><th>Đơn giá (đ/kg)</th><th>Phí vận chuyển</th><th>Thành tiền</th><th>Hành động</th></tr></thead><tbody>{rows.length ? rows.map((record,index)=><tr key={record.id}><td>{index+1}</td><td><b>{record.title}</b></td><td>{String(record.data.quantity)}</td><td>{String(record.data.unit)}</td><td>{String(record.data.weight)}</td><td>{money(Number(record.data.unitPrice))}</td><td>{money(Number(record.data.shipping))}</td><td className="money-green"><b>{money(Number(record.data.amount))}</b></td><td><div className="row-actions"><ActionButton onClick={()=>begin(record)}><Edit3 size={15}/></ActionButton><ActionButton className="danger" onClick={()=>remove(record)}><Trash2 size={15}/></ActionButton></div></td></tr>) : <tr><td colSpan={9} className="empty-cell">{query ? "Không tìm thấy mặt hàng phù hợp." : "Chưa có phiếu nhập hàng."}</td></tr>}</tbody></table></div></section>
    <section className="table-card"><div className="table-head"><h2>Lịch sử nhập hàng</h2><div className="ref-tabs compact"><ActionButton className="active">Tất cả</ActionButton><ActionButton>Hôm nay</ActionButton><ActionButton>Tháng này</ActionButton></div></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Ngày nhập</th><th>Mặt hàng</th><th>Số lượng</th><th>Cân nặng</th><th>Thành tiền</th><th>Người nhập</th></tr></thead><tbody>{rows.length ? rows.slice(0,5).map(r=><tr key={r.id}><td>{String(r.data.date)}</td><td>{r.title}</td><td>{String(r.data.quantity)} {String(r.data.unit)}</td><td>{String(r.data.weight)} kg</td><td><b>{money(Number(r.data.amount))}</b></td><td>{String(r.data.createdByName ?? r.data.createdBy ?? "Không có dữ liệu")}</td></tr>) : <tr><td colSpan={6} className="empty-cell">Chưa có lịch sử nhập hàng.</td></tr>}</tbody></table></div></section>
    {open&&<div className="modal-backdrop"><ActionForm className="modal goods-modal" onSubmit={save}>
      <div className="modal-title"><div><h2>{editing?"Cập nhật mặt hàng":"Thêm mặt hàng"}</h2><p>Thành tiền tự động theo cân nặng × đơn giá + vận chuyển</p></div><ActionButton type="button" onClick={()=>setOpen(false)}><X size={19}/></ActionButton></div>
      ...
      <div className="modal-actions"><ActionButton type="button" onClick={()=>setOpen(false)}>Hủy</ActionButton><ActionButton className="primary-button">Lưu mặt hàng</ActionButton></div>
    </ActionForm></div>}
  </div>;
}
...
function AttendanceManagement({ store }: { store: ReferenceStore }) {
  ...
  return <div className="reference-module attendance-page">
    <div className="ref-toolbar"><div><h2>Chấm công</h2><p>Lịch sử ca làm, điểm danh và thống kê lương theo giờ thực tế</p></div><div className="ref-toolbar-actions">
      <ActionButton type="button" onClick={() => csv("cham-cong.csv", [["Ngày", "Nhân viên", "Ca", "Điểm danh", "Vị trí điểm danh", "Phụ cấp TikTok", "Số giờ", "Lương cứng/giờ", "Lương thực nhận", "Trạng thái"], ...rows.map((row) => [row.workDate, row.employeeName, row.shiftNames.join(", "), attendanceLabel(row), locationExportLabel(row.locations), row.tiktokAllowance, (row.durationSeconds / 3_600).toFixed(2), rowRate(row), row.salary, shiftState(row).label])])}><Download size={16}/> Xuất Excel</ActionButton>
    </div></div>
    ...
    <section className="table-card attendance-records" aria-labelledby="attendance-records-title">
      <div className="table-head attendance-table-head"><div className="attendance-table-title"><h2 id="attendance-records-title">LỊCH SỬ CHẤM CÔNG</h2><p>Chọn thời gian và cách tổng hợp để xem đúng dữ liệu theo ca, ngày hoặc từng nhân viên.</p></div><div className="attendance-history-filters"><div className="attendance-time-filter">{mode === "shift"
        ...
        : <DatePickerControl ariaLabel="Tháng xem lịch sử chấm công" hint="Thời gian" type="month" value={month} onChange={setMonth}/>}</div><div className="ref-tabs compact attendance-mode-tabs" role="group" aria-label="Cách tổng hợp lịch sử chấm công"><ActionButton type="button" className={mode === "shift" ? "active" : ""} aria-pressed={mode === "shift"} onClick={() => setMode("shift")}>Theo ca</ActionButton><ActionButton type="button" className={mode === "day" ? "active" : ""} aria-pressed={mode === "day"} onClick={() => setMode("day")}>Theo ngày</ActionButton><ActionButton type="button" className={mode === "month" ? "active" : ""} aria-pressed={mode === "month"} onClick={() => setMode("month")}>Theo nhân viên</ActionButton></div><label className="ref-search"><Search size={16} aria-hidden="true"/><input aria-label="Tìm nhân viên hoặc ca" placeholder="Tìm nhân viên hoặc ca..." value={query} onChange={(event) => setQuery(event.target.value)}/></label></div><p className="attendance-guidance">Trạng thái đi sớm, đúng giờ hoặc đi trễ dùng ngưỡng đã lưu tại thời điểm nhân viên điểm danh. Vị trí là ảnh chụp định vị tại thời điểm nhân viên xác nhận điểm danh.</p></div>
    ...
    {adjustment ? <div className="modal-backdrop" ref={adjustmentBackdropRef}>
      ...
      >
        <div className="modal-title"><div><h2 id="attendance-adjustment-title">Điều chỉnh chấm công</h2><p>{adjustment.employeeLabel}<br/>{adjustment.shiftLabel} · {dateLabel(adjustment.workDate)}</p></div><ActionButton type="button" aria-label="Đóng hộp thoại" disabled={adjustmentSaving} onClick={closeAdjustment}><X size={19}/></ActionButton></div>
        ...
        <div className="modal-actions"><ActionButton type="button" disabled={adjustmentSaving} onClick={closeAdjustment}>Hủy</ActionButton><ActionButton className="primary-button" disabled={adjustmentSaving}>{adjustmentSaving ? "Đang lưu…" : "Lưu điều chỉnh"}</ActionButton></div>
      </ActionForm>
    </div> : null}
...
function PayrollManagement({ store, period, onPeriodChange }: { store: ReferenceStore; period?: string; onPeriodChange?: (period: string) => void }) {
  ...
  return <div className="reference-module payroll-page">
    <div className="ref-toolbar"><div><h2>Lương thưởng nhân viên</h2><p>Tổng kết lương và thưởng KPI theo giờ làm thực tế của từng cửa hàng</p></div><div className="ref-toolbar-actions payroll-compact-actions">
      ...
      <ActionButton disabled={!dataIsCurrent} onClick={exportPayroll}><Download size={16}/> Xuất Excel</ActionButton>
      <ActionButton disabled={locked || loading || finalizing || !dataIsCurrent} onClick={() => begin("ALLOWANCE")}><Plus size={16}/> Tạo phụ cấp</ActionButton>
      <ActionButton disabled={locked || loading || finalizing || !dataIsCurrent} onClick={() => begin("BONUS")}><Gift size={16}/> Tạo thưởng</ActionButton>
      <ActionButton className="primary-button" disabled={locked || loading || finalizing || !dataIsCurrent} onClick={finalize}><CheckCircle2 size={16}/>{locked ? "Đã khóa kỳ" : loading || finalizing ? "Đang tính..." : "Tổng kết tháng"}</ActionButton>
    </div></div>
    ...
    <section className="table-card"><div className="table-head"><h2>Lịch sử tạo phụ cấp và thưởng · {formatMonthVn(month)}</h2><span>{periodRecords.length} bản ghi</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Thời gian</th><th>Nhân viên</th><th>Loại</th><th>Số tiền</th><th>Nội dung chi</th><th>Người tạo</th><th>Thao tác</th></tr></thead><tbody>
      {periodRecords.length ? periodRecords.map((record) => <tr key={record.id}><td>{dateLabel(String(record.data.date))}</td><td><b>{String(record.data.employeeName)}</b></td><td><span className={record.data.kind === "BONUS" ? "bonus-pill" : "allowance-pill"}>{record.data.kind === "BONUS" ? "Thưởng" : "Phụ cấp"}</span></td><td className="money-green"><b>{money(Number(record.data.amount))}</b></td><td>{String(record.data.note || "—")}</td><td>{String(record.data.createdByName ?? "Tài khoản đã xóa")}</td><td><ActionButton disabled={locked || loading || finalizing || !dataIsCurrent || deletingAdjustmentId !== null} className="danger-link" onClick={() => remove(record.id)}>{deletingAdjustmentId === record.id ? "Đang xóa…" : "Xóa"}</ActionButton></td></tr>) : <tr><td colSpan={7} className="empty-cell">Chưa phát sinh phụ cấp hoặc thưởng trong kỳ.</td></tr>}
    </tbody></table></div></section>
    ...
    {open && adjustmentScope && adjustmentIsCurrent && <div ref={payrollBackdropRef} className="modal-backdrop"><ActionForm ref={payrollDialogRef} className="modal payroll-action-modal" role="dialog" aria-modal="true" aria-labelledby="payroll-adjustment-dialog-title" aria-busy={savingAdjustment} tabIndex={-1} onSubmit={save}><div className="modal-title"><div><h2 id="payroll-adjustment-dialog-title">{kind === "ALLOWANCE" ? "Tạo phụ cấp" : "Tạo thưởng"}</h2><p>Khoản chi được ghi nhận đúng nhân viên, cửa hàng và tháng lương</p></div><ActionButton type="button" aria-label="Đóng hộp thoại tạo phụ cấp hoặc thưởng" disabled={savingAdjustment} onClick={closeAdjustmentDialog}><X size={19}/></ActionButton></div><label>Nhân viên được nhận *<select ref={payrollEmployeeSelectRef} disabled={savingAdjustment} value={employeeId} onChange={(event) => setEmployeeId(event.target.value)}>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.code} · {employee.name}{employeeStatusSuffix(employee.status)}</option>)}</select></label><label>Số tiền {kind === "ALLOWANCE" ? "phụ cấp" : "thưởng"} *<input inputMode="numeric" required disabled={savingAdjustment} value={amount} onChange={(event) => setAmount(formatVndInput(event.target.value))} placeholder="0"/><small>Nhập theo VND, ví dụ: 15000 sẽ hiển thị 15,000.</small></label><div className="app-date-picker-field"><span>Ngày ghi nhận</span><DatePickerControl ariaLabel="Ngày ghi nhận phụ cấp hoặc thưởng" min={`${adjustmentScope.period}-01`} max={`${adjustmentScope.period}-31`} disabled={savingAdjustment} value={date} onChange={setDate}/></div><label>Nội dung chi *<textarea required disabled={savingAdjustment} value={note} onChange={(event) => setNote(event.target.value)} placeholder={kind === "ALLOWANCE" ? "Ví dụ: Phụ cấp chuyên cần" : "Ví dụ: Thưởng hoàn thành công việc"}/></label>{message && <div className="form-message" role="alert">{message}</div>}<div className="modal-actions"><ActionButton type="button" disabled={savingAdjustment} onClick={closeAdjustmentDialog}>Hủy</ActionButton><ActionButton className="primary-button" disabled={savingAdjustment || !employeeId}>{savingAdjustment ? "Đang lưu…" : `Lưu ${kind === "ALLOWANCE" ? "phụ cấp" : "thưởng"}`}</ActionButton></div></ActionForm></div>}
  </div>;
}
...
function CashflowManagement({ store }: { store: ReferenceStore }) {
  ...
  async function remove(id:string){if(await deleteRecord(id))await reload();}
  return <div className="reference-module cashflow-page"><div className="ref-toolbar"><div><h2>Dòng tiền</h2><p>Theo dõi doanh thu, chi phí và lợi nhuận cửa hàng</p></div><div className="ref-toolbar-actions"><input type="date" defaultValue={today()}/><ActionButton onClick={()=>csv("dong-tien-cua-hang.csv",[["Ngày","Loại chi phí","Số tiền","Ghi chú"],...records.map(r=>[String(r.data.date),r.title,Number(r.data.amount),String(r.data.note)])])}><Download size={16}/> Xuất Excel</ActionButton><ActionButton className="primary-button" onClick={()=>begin()}><Plus size={17}/> Thêm chi phí</ActionButton></div></div>
    ...
    <section className="table-card"><div className="table-head"><h2>Chi phí cố định gần đây</h2><span>{records.length} khoản phát sinh</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Ngày</th><th>Loại chi phí</th><th>Số tiền</th><th>Ghi chú</th><th>Người tạo</th><th>Thao tác</th></tr></thead><tbody>{records.length?records.map(record=><tr key={record.id}><td>{String(record.data.date)}</td><td><b>{record.title}</b></td><td className="money-orange">{money(Number(record.data.amount))}</td><td>{String(record.data.note||"—")}</td><td>{String(record.data.createdByName ?? record.data.createdBy ?? "Không có dữ liệu")}</td><td><div className="row-actions"><ActionButton onClick={()=>begin(record)}><Edit3 size={15}/></ActionButton><ActionButton className="danger" onClick={()=>remove(record.id)}><Trash2 size={15}/></ActionButton></div></td></tr>):<tr><td colSpan={6} className="empty-cell">Chưa nhập chi phí phát sinh.</td></tr>}</tbody></table></div></section>
    {open&&<div className="modal-backdrop"><ActionForm className="modal" onSubmit={save}><div className="modal-title"><div><h2>{editing?"Cập nhật chi phí":"Thêm chi phí"}</h2><p>Dữ liệu được ghi riêng cho {store.name}</p></div><ActionButton type="button" onClick={()=>setOpen(false)}><X size={19}/></ActionButton></div><label>Loại chi phí<select value={type} onChange={(e)=>setType(e.target.value)}><option>Marketing</option><option>Setup</option><option>Mặt bằng</option><option>Điện</option><option>Nước</option><option>Wifi</option><option>Rác</option><option>Khác</option></select></label><div className="form-grid two"><label>Số tiền *<input type="text" inputMode="numeric" pattern="[0-9,]*" required value={amount} onChange={(e)=>setAmount(formatVndInput(e.target.value))}/></label><label>Ngày chi<input type="date" value={date} onChange={(e)=>setDate(e.target.value)}/></label></div><label>Nội dung chi *<textarea required value={note} onChange={(e)=>setNote(e.target.value)}/></label>{message&&<div className="form-message">{message}</div>}<div className="modal-actions"><ActionButton type="button" onClick={()=>setOpen(false)}>Hủy</ActionButton><ActionButton className="primary-button">Lưu chi phí</ActionButton></div></ActionForm></div>}
  </div>;
}
