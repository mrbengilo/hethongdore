...
type EmployeeTransfer = { has_open_shift?: number; id: string; employee_id: string; employee_code: string; employee_name: string; employee_position: string; source_store_id: string; source_store_name: string; target_store_id: string; target_store_name: string; start_date: string; end_date: string; shifts: string[]; support_hourly_rate: number; support_allowance: number; reason: string; status: "SCHEDULED" | "ACTIVE" | "COMPLETED" | "CANCELLED"; created_by_name?: string };
...
export function ReferenceManagerTransfer({ stores }: { stores: ReferenceStore[] }) {
  ...
  return <div className="page-content manager-reference"><div className="transfer-reference-grid transfer-new-layout">
    ...
    <section className="transfer-policy-card"><h2>3. QUYỀN TRUY CẬP HỆ THỐNG</h2><p><ShieldCheck/> <span><b>Được đăng nhập hệ thống của cửa hàng hỗ trợ</b><small>Chuyển sang điểm danh tại cửa hàng hỗ trợ khi đến lịch và đã kết ca tại cửa hàng đang làm việc.</small></span></p><p><XCircle/> <span><b>Trả về cửa hàng chính sau khi kết ca hỗ trợ</b><small>Hết giờ hỗ trợ vẫn tiếp tục làm việc tại cửa hàng hỗ trợ cho đến khi kết ca.</small></span></p></section>
    ...
    <section className="manager-panel table-panel"><div className="panel-title"><h2>LỊCH SỬ ĐIỀU CHUYỂN</h2><span>{transfers.length} bản ghi</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Thời gian hỗ trợ</th><th>Nhân viên</th><th>Cửa hàng điều đi</th><th>Cửa hàng hỗ trợ</th><th>Ca làm việc</th><th>Lương/giờ</th><th>Phụ cấp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{transfers.length === 0 ? <tr><td colSpan={9} className="empty-cell">Chưa có lịch sử điều chuyển.</td></tr> : transfers.map((record) => <tr key={record.id}><td>{record.start_date} → {record.end_date}</td><td><b>{record.employee_code} · {record.employee_name}</b><SupportTag supporting sourceStoreName={record.source_store_name}/></td><td>{record.source_store_name}</td><td>{record.target_store_name}</td><td>{record.shifts.join(", ") || "—"}</td><td>{money(record.support_hourly_rate)}</td><td>{money(record.support_allowance)}</td><td><span className={`status-pill transfer-${record.status.toLowerCase()}`}>{statusLabel(record.status)}</span></td><td><div className="row-actions">{!["COMPLETED", "CANCELLED"].includes(record.status) && <><ActionButton disabled={Boolean(record.has_open_shift)} title={record.has_open_shift ? "Nhân viên phải kết ca hỗ trợ trước" : undefined} onClick={() => updateStatus(record, "END")}>Kết thúc</ActionButton><ActionButton className="danger" disabled={Boolean(record.has_open_shift)} onClick={() => updateStatus(record, "CANCEL")}>Hủy</ActionButton></>}</div></td></tr>)}</tbody></table></div></section>
  </div>;
}
