...
function conflictResponse(error: SalaryAdvanceConflictError) {
  ...
  if (error.reason === "LOCKED") return noStoreJson({ message: "Kỳ lương đã bắt đầu chốt hoặc đã khóa; không thể thay đổi khoản ứng lương." }, 423);
  if (error.reason === "LIMIT") return noStoreJson({ message: "Số tiền ứng phải nhỏ hơn số tiền khả dụng của nhân viên trong kỳ lương này." }, 409);
