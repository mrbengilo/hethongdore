...
import { ensureActiveShiftRollover } from "../_lib/shift-rollover";
...
export async function GET(request: Request) {
  ...
  const shift = await ensureActiveShiftRollover(user, db);
  if (!shift.active || !shift.shiftCode || !user.employeeId || !user.storeId) {
    return json({ orders: [], active: false, shift, message: "Bạn chưa bắt đầu ca làm việc" });
  }
  result = await db.prepare("SELECT o.*, e.name AS employeeName FROM orders o JOIN employees e ON e.id = o.employee_id WHERE o.store_id = ? AND o.employee_id = ? AND o.shift_code = ? ORDER BY o.created_at DESC")
    .bind(user.storeId, user.employeeId, shift.shiftCode).all();
  return json({ orders: result.results, active: true, shift });
}
...
export async function POST(request: Request) {
  ...
  const db = await initDb();
  const shift = await ensureActiveShiftRollover(user, db);
  if (!shift.active || !shift.shiftCode || !user.employeeId || !user.storeId) return json({ message: "Bạn chưa bắt đầu ca làm việc" }, 409);
  ...
  await db.prepare("INSERT INTO orders (id, code, store_id, employee_id, shift_code, customer_name, phone, age, amount, payment_method, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'COMPLETED', ?)")
    .bind(id, code, user.storeId, user.employeeId, shift.shiftCode, body.customerName?.trim() || null, body.phone?.trim() || null, age, amount, body.paymentMethod, new Date().toISOString()).run();
  await writeAudit(user.id, "CREATE", "ORDER", id, `${code}; shift=${shift.shiftCode}`);
  return json({ id, code, shift });
}
...
export async function PATCH(request: Request) {
  ...
  const order = await db.prepare("SELECT id, code FROM orders WHERE id = ? AND store_id = ? AND employee_id = ? AND shift_code = ? AND status = 'COMPLETED'")
    .bind(body.id, user.storeId, user.employeeId, shift.shiftCode).first<{ id: string; code: string }>();
  ...
  await db.prepare("UPDATE orders SET customer_name = ?, phone = ?, age = ?, amount = ?, payment_method = ? WHERE id = ? AND store_id = ? AND employee_id = ? AND shift_code = ? AND status = 'COMPLETED'")
    .bind(body.customerName?.trim() || null, body.phone?.trim() || null, age, amount, body.paymentMethod, body.id, user.storeId, user.employeeId, shift.shiftCode).run();
  await writeAudit(user.id, "UPDATE", "ORDER", body.id, order.code);
  return json({ ok: true, code: order.code, shift });
}

export async function DELETE(request: Request) {
  const user = await getSessionUser(request);
  if (!user || user.role !== "EMPLOYEE") return json({ message: "Không thể hủy đơn ngoài ca hiện tại." }, 403);
  const db = await initDb();
  const shift = await ensureActiveShiftRollover(user, db);
  if (!shift.active || !shift.shiftCode || !user.employeeId || !user.storeId) return json({ message: "Không thể hủy đơn ngoài ca hiện tại." }, 403);
  const id = new URL(request.url).searchParams.get("id");
  const order = await db.prepare("SELECT * FROM orders WHERE id = ? AND store_id = ? AND employee_id = ? AND shift_code = ? AND status = 'COMPLETED'")
    .bind(id, user.storeId, user.employeeId, shift.shiftCode).first();
  if (!order) return json({ message: "Không tìm thấy đơn thuộc ca hiện tại." }, 404);
  await db.prepare("UPDATE orders SET status = 'VOID' WHERE id = ? AND store_id = ? AND employee_id = ? AND shift_code = ? AND status = 'COMPLETED'")
    .bind(id, user.storeId, user.employeeId, shift.shiftCode).run();
