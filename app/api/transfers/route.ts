...
async function reconcileStatuses() {
  ...
  const activating = await db.prepare("SELECT id FROM employee_transfers WHERE status = 'SCHEDULED' AND start_date <= ? AND end_date >= ?").bind(day, day).all<{ id: string }>();
  ...
  if (activating.results.length) {
    await db.prepare("UPDATE employee_transfers SET status = 'ACTIVE', updated_at = ? WHERE status = 'SCHEDULED' AND start_date <= ? AND end_date >= ?").bind(now, day, day).run();
  }
