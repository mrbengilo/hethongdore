/** Additive storage: setup repayments do not alter locked store financial facts. */
export const profitSetupRepaymentStatements = [
  ...
  ...(["INSERT", "UPDATE"] as const).map((operation) => `CREATE TRIGGER IF NOT EXISTS trg_profit_setup_repayments_writable_${operation.toLowerCase()}
    BEFORE ${operation} ON profit_setup_repayments
    BEGIN
